...
export function BookPersonalEnergogram({ projection, birthDate }: BookPersonalEnergogramProps) {
  ...
  return (
    <div className="mx-auto flex min-w-[74rem] max-w-[78rem] flex-col gap-4 bg-white px-4 py-2">
      ...
      <svg viewBox={`0 0 ${width} ${height}`} className="block h-auto w-full min-w-[72rem]" role="img" aria-label={`Личная энергограмма для ${birthDate}`}>
        <image
          href={manImage}
          x={bodyX}
          y={imageY}
        ...
        {labels.map((value) => {
          ...
          return (
            <g key={`axis-${value}`}>
              ...
              <text x={x} y={48} textAnchor="middle" className="fill-stone-900 text-[28px] font-bold">
                {value}
              </text>
            </g>
          );
        ...
        {reversedOrder.map((houseId, rowIndex) => {
          const y = yPositions[rowIndex] + 8;
          return (
            <g key={`labels-${houseId}`}>
              <text x={leftDigitsWidth - 14} y={y} textAnchor="end" className="fill-stone-900 text-[32px] font-bold">
                {houseId}
              </text>
              <text x={chartRight + rightDigitsWidth / 2} y={y} textAnchor="middle" className="fill-stone-700 text-[32px] font-bold">
                {rowIndex + 1}
              </text>
            </g>
          );
        ...
        <text x={leftDigitsWidth - 14} y={chartTop + chartHeight + 36} textAnchor="end" className="fill-stone-500 text-[18px] font-bold uppercase tracking-[0.24em]">
          дома
        </text>
      </svg>
    </div>
